...
class CommandExecutor:
    ...
    def execute(self, text):
        ...
        if cmd == 'quit':
            self.iface.mainWindow().close(); return
        if cmd == 'open_file':
            return self.open_file()
        if cmd == 'jimok_cleanup':
            return self.jimok_cleanup()
        if cmd == 'eia_wfs': return self.api.load_eia_bsnsarea_wfs()
        if cmd == 'ecology_wfs': return self.api.load_ecology_wfs()
        ...
        if cmd == 'cadastral_stats':
            self.log('지목별 면적 집계는 jimok_cls 필드 정리 후 면적 집계 모듈과 연결할 예정입니다.')
            return
        if cmd == 'zoomout_10km':
    ...
    def open_file(self):
        path, _ = QFileDialog.getOpenFileName(self.iface.mainWindow(), '벡터 파일 열기', '', 'Vector files (*.shp *.gpkg *.geojson *.dxf);;All files (*.*)')
        if not path: return
    ...
    def jimok_cleanup(self):
        ...
        if not layer:
            ...
            return
        n = cleanup_jimok(layer)
        self.log('지목 정리 필드 jimok_cls를 생성/갱신했습니다. 처리 건수: %s' % n)
